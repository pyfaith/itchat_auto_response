...
#缓存文件保存位置
_TEMP_PATH = "{sep}tmp{sep}weitchat{sep}".format(sep=os.sep)
...
def is_company_wx(msg):
    ...
    if hasattr(msg, "User"):
        nickname = msg["User"]['NickName'] \
            if hasattr(msg["User"],"NickName") else None
...
def download_files(msg):
    if is_company_wx(msg):
        ...
        msg['Text'](get_temp_path() +  msg['FileName'])      #下载文件
        #将下载的文件发送给发送者
        itchat.send('@%s@%s' % (
                                'img' if msg['Type'] == 'Picture' else 'fil',
                                get_temp_path() + msg["FileName"]
                                ),
                    msg["FromUserName"]
                    )
